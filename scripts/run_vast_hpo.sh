...
python -u scripts/tune_diverse_ray.py --root "$root" --mode hpo --search random \
  ...
  --name hpo_diverse_v1 > "$root/hpo_driver.log" 2>&1
python scripts/select_and_export_tuning.py --root "$root" --select-only \
  > "$root/hpo_selection.log" 2>&1
python -u scripts/tune_diverse_ray.py --root "$root" --mode ablation \
  --best-config "$root/runs/hpo_diverse_v1_best_config.json" --report-to wandb \
  --name ablation_diverse_v1 > "$root/ablation_driver.log" 2>&1
